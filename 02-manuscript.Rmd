...
run_ani_spatial_analysis <- function(bin_name, ani_list, ids_35day, meta_data) {
  ...
  # Mantel correlogram
  mantel_res <- vegan::mantel.correlog(popani_dist, spatial_dist, nperm = 999, cutoff = TRUE, n.class = 15, r.type = "spearman")
  plot(mantel_res, main = paste("Mantel correlogram:", bin_name))
...
names(ani_list)
# Example bin names you want to analyze
bins_to_run <- c("GPB_bin_000044","GPB_bin_000047","GPB_bin_000063")  # add your list here

# Metadata for filtering ids (you already have this)
ids_35day <- lawsonibacter_popANI_meta %>%
  filter(animal == "G121e") %>%
  pull(microsample)

# coord_data = macroquant_metrics_filt_30 with microsample, Xcoord, Ycoord
...
supfigs7 <- cowplot::plot_grid(results_list$GPB_bin_000044$plot, 
                            results_list$GPB_bin_000047$plot, 
                            results_list$GPB_bin_000063$plot,
  ncol = 1, label_size = 16
)

# ggsave(filename = "figures/lawsonib_distance.png", supfigs7, width = 8, height = 10)

results_list$GPB_bin_000044$plot
results_list$GPB_bin_000047$plot
results_list$GPB_bin_000063$plot
...
vegan::mantel.correlog(popani_distance_lmp,spatial_distance_lmp,nperm = 999,cutoff = TRUE,n.class = 25,r.type = "spearman")
plot(vegan::mantel.correlog(popani_distance_lmp,spatial_distance_lmp,nperm = 999,cutoff = TRUE,n.class = 25,r.type = "spearman"))

aovperm(lmperm(comm_dist~spat_dist,data=toplot),np=10000)
summary(lmperm(comm_dist~spat_dist,data=toplot),np=10000)
