...
## Load data
```{r load_data_main_manuscript, message=FALSE, eval=FALSE}
load("data/25_07_09_selected_data.Rdata") # load the data from 1-data_preparation
```

## Microbial Genomic Reference Data

### Macro-scale MAGs Catalogue Summary Statistics 
```{r stat_macromag, message=FALSE, eval=FALSE}
...
```{r stat_micromag, message=FALSE, eval=FALSE}
...
```{r fig_2a, message=FALSE, eval=FALSE}
```

### Amalia: Macro MAGs Overview (Fig. 2b)

```{r fig_2b, message=FALSE, eval=FALSE}
```

### Amalia: Micro MAGs Overview (Suppl. Fig. 1)

```{r suppl_fig_1, message=FALSE, eval=FALSE}
```

### Comparative Analysis of Genome Catalogues (Suppl. Note 1)

```{r suppl_note_1, message=FALSE, eval=FALSE}
...
```{r suppl_note_1_samples, message=FALSE, eval=FALSE}
...
#### Read Counts 
```{r SN1_read_count, message=FALSE, eval=FALSE}
...
```{r sn1_alpha_df, message=FALSE, eval=FALSE}
...
```{r FigS1_1a, message=FALSE, eval=FALSE}
...
```{r sn1_alpha_unique_species, message=FALSE, eval=FALSE}
...
```{r sn1_alpha_unique_genera, message=FALSE, eval=FALSE}
...
```{r sn1_beta_dfs, message=FALSE, eval=FALSE}
...
```{r FigS1_1b, message=FALSE, eval=FALSE}
...
```{r FigS1_1c, message=FALSE, eval=FALSE}
...
#### Fig. S1
```{r FigS1, message=FALSE, eval=FALSE}
...
```{r sn3_2_df, message=FALSE, eval=FALSE}
...
```{r FigS3_2_2, message=FALSE, eval=FALSE}
ordered_metrics <- c(
...
```{r sn3_2_a_caecum, message=FALSE, eval=FALSE}
...
```{r sn3_2_a_colon, message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
```{r sn3_2_b_caecum, message=FALSE, eval=FALSE}
...
```{r sn3_2_b_colon, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r sn3_2_c_caecum, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r sn3_2_c_colon, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
###### d. Richness
```{r sn3_2_d_caecum, message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
```{r sn3_2_d_colon, message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
###### e. Neutral" = "neutral",
```{r sn3_2_e_caecum, message=FALSE, eval=FALSE}
...
```{r sn3_2_e_colon, message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
###### f. Phylogenetic" = "phylogenetic"
```{r sn3_2_f_caecum, message=FALSE, eval=FALSE}
# # Continuous floats (any real number): lm or gaussian, Gamma (continuous positive data, skewed to the right)
...
```{r sn3_2_f_colon, message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
```{r sn3_2_taxa, message=FALSE, eval=FALSE}
...
```{r sn3_3_df, message=FALSE, eval=FALSE}
...
```{r FigS3_3_1, message=FALSE, eval=FALSE}
ordered_metrics <- c(
  "total_after_filtering.total_reads",
...
###### a. Reads Retained Post Quality Filtering
```{r sn3_3_a, message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
```{r sn3_3_b, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r sn3_3_c, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r d_sectioncomp, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r e_sectioncomp, message=FALSE, eval=FALSE}
...
```{r Boxplot_19versus15Cycles, message=FALSE, eval=FALSE}
ordered_metrics <- c(
...
```{r reads_cyclecomp , message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
```{r readsfilt_cyclecomp , message=FALSE, eval=FALSE}
# Counts (integers ≥0): poisson, quasipoisson (accounts overdispersion)
...
```{r quality_cyclecomp, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r duplication_cyclecomp, message=FALSE, eval=FALSE}
# Proportions (0–1 continuous):	quasibinomial GLM (accounts overdispersion)
...
```{r physeq_section, message=FALSE, eval=FALSE}
...
```{r physeq_section_lg, message=FALSE, eval=FALSE}
...
```{r figs3_3_2, message=FALSE, eval=FALSE}
...
```{r sn3_3_dapi_df, message=FALSE, eval=FALSE}
...
```{r figs3_3_3a, message=FALSE, eval=FALSE}
...
```{r FigS3_4_1, message=FALSE, eval=FALSE}
ordered_metrics <- c(
  "total_before_filtering.total_reads",
  "lowquality_reads_pct",
  "adapter_contamination_pct",
...
```{r FigS3_4_2, message=FALSE, eval=FALSE}
```

###### FigS3_4_3

```{r FigS3_4_3, message=FALSE, eval=FALSE}
```


###### Visual Comparison of Coverage Cutoff Filtering (Fig. S3.4.4)

```{r FigS3_4_4, message=FALSE, eval=FALSE}
...
```{r sn3_4_physeq_lg, message=FALSE, eval=FALSE}
...
```{r figs3_4_5, message=FALSE, eval=FALSE}
...
```{r x3, message=FALSE, eval=FALSE}
```

### Resource optimisation and throughput (Suppl. Note 3.5)
```{r sn3_5_df, message=FALSE, eval=FALSE}
...
```{r sn3_5_total_pct, message=FALSE, eval=FALSE}
sn3_5_df_t  <- sn3_5_df %>%
...
```{r sn3_5_median_pct, message=FALSE, eval=FALSE}
sn3_5_df_t <- sn3_5_df %>%
...
```{r FigS3_5_1a, message=FALSE, eval=FALSE}
ordered_metrics <- c(
  # "total_before_filtering.total_reads",
...
```{r sn3_5_dreamprep_df , message=FALSE, eval=FALSE}
...
```{r FigS3_5_1b, message=FALSE, eval=FALSE}
...
```{r FigS3_5_1, message=FALSE, eval=FALSE}
...
#### Data selection
```{r mssm_discriminative_df, message=FALSE, eval=FALSE}
...
```{r mssm_discriminative_object, warning=FALSE, comments="", message=FALSE, eval=FALSE}
...
```{r mssm_discriminative_metadata, message=FALSE, eval=FALSE}
...
```{r mssm_discriminative_betadispersion_animal, message=FALSE, eval=FALSE}
...
```{r mssm_discriminative_betadispersion_cryosection, message=FALSE, eval=FALSE}
# Perform betadisper analysis
betadisper_animal <- vegdist(mssm_discriminative_obj$df_clr_dist, method = "euclidean") %>%
...
```{r mssm_discriminative_betadispersion_animal&cryosection, message=FALSE, eval=FALSE}
...
```{r mssm_discriminative_permanova, message=FALSE, eval=FALSE}
...
```{r supplementary_figure_5, message=FALSE, eval=FALSE}
...
```{r mivsma_ra, message=FALSE, eval=FALSE}
...
```{r mivsma_ra_nrmse, message=FALSE, eval=FALSE}
...
```{r mivsma_clr, message=FALSE, eval=FALSE}
...
```{r all_overlap, message=FALSE, eval=FALSE}
...
###### Shared Genera 
```{r mivsma_genus, message=FALSE, eval=FALSE}
...
```{r mivsma_35d_genus, message=FALSE, eval=FALSE}
...
```{r mivsma_14d_genus, message=FALSE, eval=FALSE}
...
```{r sup_table_1, message=FALSE, eval=FALSE}
...
```{r fig5a, message=FALSE, eval=FALSE}
total_microsamples <- length(unique(mivsma_phylo_lg %>% filter(group %in% c("Micro-scale Spatial Metagenomics"),
                           animal %in% c("G103b"),
                           !section %in%c("Colon", "Ileum")) %>% pull(microsample)))
...
```{r figs4_2, message=FALSE, eval=FALSE}
...
ggsave(filename = "mssm_figures/Fig6_a_legend.png", 
...
```{r lawsonibacter_mantel_G121e , eval=FALSE}
...
```{r lawsonibacter_mantel_G103b , eval=FALSE}
...
```{r lawsonibacter_popANI, eval=FALSE}
...
###### Strategy 2 
```{r distance_decay_2, eval=FALSE}
ani_distance_G121eI103A <- popani_distance_lmp  %>% 
  as.matrix() %>% as.data.frame() %>% 
  rownames_to_column("Sample1") %>%
  pivot_longer(-Sample1, names_to = "Sample2", values_to = "ANI")

spatial_distance_G121eI103A <- spatial_distance_lmp  %>% 
  as.matrix() %>% as.data.frame() %>% 
  rownames_to_column("Sample1") %>%
...
ani_distance_G121eI103A %>% 
  ...
  filter(!Sample1==Sample2) %>% 
  pull(Distance) %>% 
  hist(main = "Histogram of Spatial Distances", xlab = "Distance (μm)")

predict(ani_lm, newdata = tibble(Distance = c(1:500)), interval = "prediction", level = 0.95) %>% 
  mean()

predict(ani_lm, newdata = tibble(Distance = c(3000:3500)), interval = "prediction", level = 0.95) %>% 
  mean()

predict(ani_lm, newdata = tibble(Distance = c(1:4000)), interval = "prediction", level = 0.95) %>% data.frame %>%
  mutate(Distance = 1:4000) %>% 
  ggplot(aes(x = Distance, y = fit)) +
  geom_line(color = "blue") +                         # Fitted line
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.2, fill = "blue") +  # Prediction interval
  labs(title = "Predicted Values with 95% Prediction Interval",
       x = "Distance",
       y = "Predicted ANI") +
  theme_minimal()
```
