...
### run_ani_spatial_analysis()
run_ani_spatial_analysis <- function(bin_name, ani_list, ids_35day, meta_data) {
  
  # Subset ANI matrix to samples of interest
  popani <- ani_list[[bin_name]][rownames(ani_list[[bin_name]]) %in% ids_35day, ]
  popani <- popani[, colnames(popani) %in% ids_35day]  # keep only matching columns too
  diag(popani) <- NA
  ...
  p_cor <- plot(mantel_res, main = paste("Mantel correlogram:", bin_name))
  #print(p_cor)
